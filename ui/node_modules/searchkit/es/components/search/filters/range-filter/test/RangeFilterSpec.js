...
describe('Range Filter tests', () => {
    beforeEach(() => {
        ...
        this.createWrapper = ({ withHistogram = true, interval = undefined, rangeComponent = RangeSliderHistogram }) => {
            this.wrapper = mount(React.createElement(RangeFilter, { id: "m", searchkit: this.searchkit, field: "metascore", min: 0, max: 100, title: "metascore", interval: interval, translations: { 'range.divider': ' to ' }, rangeFormatter: this.rangeFormatter, rangeComponent: rangeComponent, showHistogram: withHistogram }));
    ...
    it('renders without histogram', () => {
        ...
        expect(this.wrapper.find('.sk-range-histogram')).toHaveLength(0);
        expect(this.wrapper.find('.sk-range-histogram__bar')).toHaveLength(0);
        expect(this.wrapper).toMatchSnapshot();
    });
    ...
    it('renders with range input component', () => {
        ...
        expect(this.wrapper).toMatchSnapshot();
        this.wrapper.find("input[placeholder='min']").simulate('change', { target: { value: 20 } });
        this.wrapper.find("input[placeholder='max']").simulate('change', { target: { value: 80 } });
        this.wrapper.find('form').simulate('submit');
        const query = this.searchkit.buildQuery();
        expect(query.getSelectedFilters()[0].value).toEqual('20 score to 80 score');
    });
});
