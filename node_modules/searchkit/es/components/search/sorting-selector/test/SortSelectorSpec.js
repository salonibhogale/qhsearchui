...
describe('SortingSelector tests', () => {
    beforeEach(() => {
        ...
        this.setWrapper = () => {
            this.wrapper = mount(React.createElement(SortingSelector, { searchkit: this.searchkit, options: [
                    { label: 'Relevance' },
    ...
    it('is disabled when no results', () => {
        expect(this.wrapper.children()).toHaveLength(1);
    ...
    it('select new sort option', () => {
        this.accessor.state = this.accessor.state.setValue('released_desc');
        this.setResults();
        const earlyOption = this.wrapper
            .find('select')
            .children()
            .at(2);
    ...
    it('custom mod, className, listComponent', () => {
        this.wrapper = mount(React.createElement(SortingSelector, { searchkit: this.searchkit, mod: "my-select", className: "custom-class", listComponent: Toggle, options: [
                { label: 'Relevance' },
                { label: 'Latest Releases', field: 'released', order: 'desc' },
                { label: 'Earliest Releases', field: 'released', order: 'asc', key: 'earliest' }
            ] }));
