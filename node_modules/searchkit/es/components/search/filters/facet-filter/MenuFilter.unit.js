...
describe('MenuFilter', () => {
    beforeEach(() => {
        ...
        this.wrapper = mount(React.createElement(MenuFilter, { searchkit: this.searchkit, translations: { Red: 'Red Translated' }, field: "color", title: "Color", orderKey: "_term", orderDirection: "asc", include: "title", exclude: ['n/a'], id: "color", size: 10 }));
        this.getOptionAt = (at) => this.wrapper
            .find('.sk-item-list')
            .children()
            .at(at);
    ...
    it('getSelectedItems', () => {
        ...
        expect(this.wrapper.instance().getSelectedItems()).toEqual(['$all']);
        this.accessor.state = new ArrayState([false]);
        expect(this.wrapper.instance().getSelectedItems()).toEqual([false]);
        this.accessor.state = new ArrayState(['foo', 'bar']);
        expect(this.wrapper.instance().getSelectedItems()).toEqual(['foo']);
    });
    ...
    it('should handle selection correctly', () => {
        ...
        const all = this.getOptionAt(0);
        const blue = this.getOptionAt(2);
        const green = this.getOptionAt(3);
        fastClick(blue);
