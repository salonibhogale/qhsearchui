...
describe('GroupedSelectedFilters tests', () => {
    beforeEach(() => {
        ...
        this.createWrapper = (props) => {
            this.wrapper = mount(React.createElement(GroupedSelectedFilters, Object.assign({ searchkit: this.searchkit }, props)));
        };
    ...
    it('handles remove click', () => {
        this.createWrapper();
        const elem = this.wrapper.find('.sk-filter-group-items__value').at(0);
        fastClick(elem);
    ...
    it('handles remove all click', () => {
        this.createWrapper();
        const elem = this.wrapper.find('.sk-filter-group__remove-action').at(0);
        fastClick(elem);
        expect(this.remove1).toHaveBeenCalled();
        expect(this.remove2).not.toHaveBeenCalled();
