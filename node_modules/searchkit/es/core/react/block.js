...
function toString() {
    // Add namespace
    let name = settings.ns + this.name, classList = resolveClassName(name);
    // Add modifiers
 ...
 */
function state(obj) {
    const context = copy(this), states = copy(context.states);
    extend(states, obj || {});
