...
export class FacetAccessor extends FilterBasedAccessor {
    ...
    buildSharedQuery(query) {
        ...
        const filterTerms = map(filters, (filter) => this.fieldContext.wrapFilter(TermQuery(this.options.field, filter)));
    ...
    buildOwnQuery(query) {
        ...
        return query.setAggs(FilterBucket(this.uuid, query.getFiltersWithoutKeys(excludedKey), ...this.fieldContext.wrapAggregations(TermsBucket(this.options.field, this.options.field, omitBy({
            size: this.size,
            order: this.getOrder(),
